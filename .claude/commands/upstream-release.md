...
- **`.github/workflows/ci.yml`**: drop upstream's new platform jobs (`build-linuxmusl-arm64`, `build-qemu`, `build-emscripten`) — keep only our `lint`, `build-native`, and `release` jobs. ALSO: upstream's `release` job creates the GitHub release via `gh release create` with `persist-credentials: false` on checkout but does NOT set a token, so `gh` fails to auth ("set the GH_TOKEN environment variable", exit 4) — add `env:\n  GH_TOKEN: ${{ github.token }}` to the **Create GitHub release for tag** step (the job already has `contents: write`). Keep our publish steps' `NODE_AUTH_TOKEN: ${{ secrets.NPM_TOKEN }}` + `--provenance` (upstream's lack both). This step may auto-merge without a conflict — verify it after every merge.
...
- **`lib/sharp.mjs`**: this fork only ships `linux-arm64` and `linux-x64`. Keep the binary-resolution `switch (runtimePlatform)` trimmed to just those two cases (both pointing at `@revizly/sharp-linux-*`); delete every other `case` (darwin, win32, linuxmusl, ppc64, riscv64, s390x, freebsd, linux-wasm32). Also delete the `@img/sharp-wasm32` fallback `require` block and the "Add WebAssembly-based dependencies" help lines. Simplify the x64-v2 microarch check to `runtimePlatform === "linux-x64"`. Upstream re-adds all of these on every merge with no conflict — grep for `@img/sharp` in `lib/sharp.mjs` after each merge and re-trim.
...
- **`npm/from-local-build.js`**: this runs in our release pipeline. Upstream's tree-shaking feature writes a `require.resolve('@img/sharp-libvips-${platform}/<entry>')` into the generated `index.cjs` for darwin/linux platforms — change that `@img` to `@revizly` so it references our published `@revizly/sharp-libvips-*` packages. NOTE: upstream renamed this entry over time — it was `/stub` (v1.3.1) and became `/binary` (v1.3.2, dynamically added to the libvips package.json by `populate-npm-workspace.sh`'s `patch_pkg_exports`); take whatever entry name upstream currently uses (and its variable name, e.g. `resolveLibvipsBinary`), just swap the scope to `@revizly`. It is `try/catch`-wrapped so a wrong scope won't crash, but the bundler hint silently fails. (May not show as a merge conflict — grep for `@img/sharp-libvips` after every merge and fix it.) Also delete the trailing `if (platform.startsWith('win') || platform.startsWith('wasm'))` block (the `@img/sharp-libvips-${libvipsPlatform}` copy of `versions.json`/licensing) — it is dead for this linux-only fork; when removing it, also drop the now-unused `appendFileSync` import. Upstream re-adds both on every merge — re-trim each time.
